...
def totals_tree(m):
    ...
    return tree(label(branches[0]) + label(branches[1]), branches)
...
def quadratic(x, a, b, c):
    ...
    if a == 0:
        return interval(f(l) if b >= 0 else f(u), f(u) if b >= 0 else f(l))
    x0 = -b / (2 * a)
    yl, yu, y0 = min(f(l), f(u)), max(f(l), f(u)), f(x0)
    is_in = x0 > l and x0 < u
    if a > 0:
        return interval(y0 if is_in else yl, yu)
    else:
        return interval(yl, y0 if is_in else yu)
...
def check_par():
    ...
    True
    """
    r1 = interval(2, 9) # Replace this line!
    r2 = interval(9, 2) # Replace this line!
    return r1, r2
