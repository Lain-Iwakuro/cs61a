...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[(1 - i % 2) * i // 2 + i % 2 * (len(deck) // 2 + i // 2)] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    for i in range(min(len(branches(t1)), len(branches(t2)))):
        new_branches.append(add_trees(branches(t1)[i], branches(t2)[i]))
    return tree(label(t1) + label(t2), new_branches + branches(t1)[i + 1:] + branches(t2)[i + 1:])
