...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        while not player.deck.is_empty():
            c = player.deck.draw()
            new_deck.insert(0, Card(c.name, c.attack + other_card.attack, c.defense + other_card.defense))
        player.deck = Deck(new_deck)
        op_new_deck = []
        while not opponent.deck.is_empty():
            c = opponent.deck.draw()
            if c.attack != other_card.attack and c.defense != other_card.defense:
                op_new_deck.insert(0, c)
        opponent.deck = Deck(op_new_deck)
